...
class CompanyController extends Controller
{
    ...
    public function ecommerceaddproduct()
    {
        ...
        return view('company/ecommerce-add-product', compact('products'));
    }
    ...
    {
        // Get the logged-in company user’s id
        $companyId = Auth::id();
        // Fetch only orders belonging to the logged in company
        $orders = Order::where('user_id', $companyId)
            ->with('software_Product') // eager load relations as needed
            ->orderBy('created_at', 'desc')
            ->get();
        return view('company/ecommerce-company-orders', compact('orders'));
    }
    public function ecommerceproduct()
    {
        // Get the logged-in user's id
        $companyId = Auth::id();
        // Fetch software products where user_id matches the logged-in user
        $products = SoftwareProduct::where('user_id', $companyId)
            ->orderBy('created_at', 'desc')
            ->get();
        return view('company/ecommerce-products', compact('products'));
    }
    public function faq()
    {
    ...
    //         ->with('softwareProduct') // eager load relations as needed
    //         ->orderBy('created_at', 'desc')
    //         ->get();
